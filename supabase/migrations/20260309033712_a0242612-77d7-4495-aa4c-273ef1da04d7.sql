...
-- Update spin_pokemon_roulette to support spin_refund boost
CREATE OR REPLACE FUNCTION public.spin_pokemon_roulette(p_user_id uuid, p_pokemon_data jsonb, p_region text DEFAULT 'kanto'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Check spin_refund boost: chance to refund the spin
  SELECT COALESCE(SUM(rb.boost_value), 0) INTO v_spin_refund_chance
...
-- Also update the icon for the boosts in DB
UPDATE roulette_boosts SET icon = 'rotate_ccw' WHERE id IN ('c53111e6-68ba-42f1-b99d-f6bbf5f7b31f', '957ab85c-1789-4f20-b7a1-a3a08ddef8e7');
