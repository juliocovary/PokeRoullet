-- Fix the sell_pokemon function to use correct goal_type for achievements
...
-- Fix claim_achievement_rewards to properly handle progressive achievements
...
BEGIN
  ...
  IF achievement_record.category = 'progressive' THEN
    -- For progressive: check if progress >= goal and not claimed
  ...
  IF achievement_record.category = 'progressive' THEN
    -- Progressive: increment completed_count, set new goal, reset some fields, mark as claimed
    ...
    SET completed_count = completed_count + 1,
        ...
        rewards_claimed = true,  -- Mark this stage as claimed
        updated_at = now()
