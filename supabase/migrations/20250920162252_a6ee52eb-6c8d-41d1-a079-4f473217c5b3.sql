-- Fix JSON concatenation operator issue in achievement function
...
-- Ensure all users have entries for all achievements 
INSERT INTO public.user_achievements (user_id, achievement_id, next_goal_value, rewards_claimed)
SELECT 
  p.user_id,
  a.id,
...
WHERE NOT EXISTS (
  ...
  WHERE ua.user_id = p.user_id AND ua.achievement_id = a.id
)
ON CONFLICT (user_id, achievement_id) DO NOTHING;