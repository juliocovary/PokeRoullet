-- Fix spin mission progress update: use mission category instead of mission type.
-- Type is daily/weekly in schema; spin is a mission category.
...
BEGIN
  ...
  WHERE user_id = p_user_id AND completed = false
    AND mission_id IN (SELECT id FROM missions WHERE category = 'spin');
