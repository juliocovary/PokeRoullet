...
export const useAchievements = () => {
  ...
  const loadAchievements = async () => {
    ...
    try {
      ...
      toast({
        title: t('toasts:achievements.error'),
        description: t('toasts:achievements.couldNotLoad'),
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  // Optimized version - does NOT reload achievements after every update (saves Egress)
  // Use skipReload=true for spin operations, false for manual actions
  const updateAchievementProgress = async (goalType: string, increment: number = 1, skipReload: boolean = false) => {
    if (!user) return;

    try {
      console.log(`[ACHIEVEMENTS] Updating progress for goal type: ${goalType}, increment: ${increment}, skipReload: ${skipReload}`);
      ...
      if (result?.achievements_completed && result.achievements_completed.length > 0) {
        ...
        result.achievements_completed.forEach((achievement: any) => {
          toast({
            title: t('toasts:achievements.achievementUnlocked'),
            description: t('toasts:achievements.goToAchievements', { title: achievement.title }),
            duration: 8000,
          });
        });
      }
  ...
  const claimAchievementRewards = async (achievementId: string) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        if (rewards.spins > 0) rewardText.push(`+${rewards.spins} ${t('toasts:achievements.eternalSpins')}`);
        if (rewards.badge_name) rewardText.push(`🎖️ ${rewards.badge_name}`);

        toast({
          title: t('toasts:achievements.rewardsClaimed'),
      ...
      toast({
        title: t('toasts:achievements.error'),
        description: t('toasts:achievements.couldNotClaimRewards'),
        variant: "destructive"
      });
      return false;
    }
  ...
  // Achievements are now loaded lazily when the AchievementsModal opens
  // This saves significant Egress by not loading achievements on every page load
